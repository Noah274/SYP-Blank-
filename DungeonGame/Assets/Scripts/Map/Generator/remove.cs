...
public class a : MonoBehaviour
{
    ...
    {
        string filePath = Path.Combine(Application.streamingAssetsPath, "SavedRooms_NormalRoom.json");
        
        if (File.Exists(filePath))
        {
            string json = File.ReadAllText(filePath);
            string[,,] roomBuild = JsonUtility.FromJson<string[,,]>(json);

            // Jetzt kannst du das Array "roomBuild" verwenden.
            // ...
        }
        else
        {
    ...
    public void printMap(int arrayNum, int y, int x, int distanceY, int distanceX, int mapArrayPosX, int mapArrayPosY, int roomType)
    {
        ...
        switch (roomType)
        {
            case 0:
                ...
                //tileBlock = ReadTilesInArea.ReadTile(roomBuild[arrayNum, y, x]);
    ...
    public string[,,] LoadRoomBuild()
    {
        ...
        string jsonString = PlayerPrefs.GetString("SavedRooms_NormalRoom.json");
